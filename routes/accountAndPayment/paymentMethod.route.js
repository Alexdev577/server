...
router.get("/user-option", auth(["ADMIN", "MANAGER"]), async (req, res) => {
  try {
    let filter = {};
    if (req?.user?.role === "MANAGER") {
      filter.manager = req?.user?._id;
    }
    const option = await PaymentMethodUser.find(filter);
...
router.patch("/user-option/:id", auth(["ADMIN", "USER"]), async (req, res) => {
  ...
  try {
    ...
    if (!method) {
      return res.status(404).json({ message: "Payment method is unavailable!" });
    }
