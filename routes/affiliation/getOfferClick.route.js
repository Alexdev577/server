...
router.get("/conversion-report/user", auth(["USER"]), async (req, res) => {
  const { startDate, endDate } = req.query;
...
router.get("/user/revenue", auth(["USER"]), async (req, res) => {
  try {
    ...
    const allTimeData = await AffiliationClick.aggregate(allTimePipeline);
    ...
    return res.status(200).json({
      allTimeRevenue: allTimeData?.[0]?.revenue,
...
//-----------------get Admin panel affiliation reports -----------------//
...
router.get("/quick-report/user/:userId", auth(["ADMIN", "MANAGER"]), async (req, res) => {
